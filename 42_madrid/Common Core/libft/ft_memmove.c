...
/*   Updated: 2024/01/09 15:29:45 by ismherna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Estos punteros se tratan como punteros a caracteres no firmados para facilitar 
...
- Si las regiones no se solapan, se realiza una copia de memoria de izquierda a derecha (hacia adelante). 
- Se utiliza un bucle while para copiar byte por byte desde src_cpy a dest_cpy, y ambos punteros se incrementan en cada iteración.
- Si las regiones se solapan, se realiza una copia de memoria de derecha a izquierda (hacia atrás). Se ajustan los punteros dest_cpy y src_cpy para apuntar al final de las regiones de destino y origen, respectivamente. Luego, se utiliza un bucle while similar para copiar byte por byte, pero esta vez decrementando los punteros después de cada iteración.
